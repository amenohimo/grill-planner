...
| FR-01-2 | 方面プリセット名設定 | 方面プリセットは3種（内部ID 0/1/2）に固定。各プリセットの表示名（デフォルト「左」「正面」「右」）をテキスト入力で編集可能。各方面切替区間にはこの3プリセットのいずれかを割り当てる |
| FR-01-3 | ブキ構成メモ | 4枠分のブキをドロップダウン（rowIdで選択）から選択可能（計算には影響しない） |
| FR-01-4 | スペシャル構成メモ | 4枠分のスペシャルをドロップダウン（rowIdで選択）から選択可能（計算には影響しない） |
| FR-01-5 | ターゲット順メモ | ターゲット順序を設定可能。行のシフト（上/下）操作にも対応（計算には影響しない） |
| FR-01-6 | タマヒロイ方向メモ | タマヒロイ方向に関する自由記述メモ（計算には影響しない） |
| FR-01-7 | シナリオコードメモ | シナリオ識別用コード。`Sxxx-xxxx-xxxx-xxxx` 形式へ自動正規化・バリデーション（計算には影響しない） |
| FR-01-8 | 自由メモ | 任意の自由記述メモ（計算には影響しない） |
| FR-01-9 | 表示モード切替 | アイコンのみ / テキストのみ / アイコン+テキスト を切り替え可能 |
| FR-01-10 | 方面カラーテーマ切替 | 方面区間の配色テーマ（花 / パステル / 霞）を切り替え可能 |
| FR-01-11 | ズーム | タイムラインの横倍率・縦倍率を独立して切替可能（50/75/100/150/200%） |
...
| ID | 要件 | 詳細 |
|----|------|------|
| FR-03-1 | 区間へのプリセット割当 | 各方面区間にホバーすると3プリセット選択UIが表示され、その区間に割り当てる方面プリセット（ID 0/1/2）を選択できる |
| FR-03-2 | キケン度連動 | キケン度変更時、方面区間数が自動的に再計算される。区間数が同じならプリセット割当を保持、変われば既定値にリセット |
